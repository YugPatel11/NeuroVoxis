  ...
  return (
    <div className="max-w-7xl mx-auto px-6 py-12 min-h-screen flex flex-col">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-bold text-white/90">Interview in Progress</h1>
          <p className="text-white/40 text-sm">Question {currentIdx + 1} of {questions.length}</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1">
          ...
          <div className="absolute bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-4">
            {!isRecording ? (
              <button 
                ...
                className="w-16 h-16 rounded-full bg-red-600 flex items-center justify-center hover:bg-red-500 transition-all shadow-[0_0_20px_rgba(220,38,38,0.4)]"
              >
        ...
        <div className="flex flex-col gap-6">
          <div className="p-10 rounded-[2.5rem] bg-white/[0.03] border border-white/10 backdrop-blur-sm flex-1 flex flex-col justify-center relative overflow-hidden">
          ...
          <div className="p-8 rounded-3xl bg-white/[0.02] border border-white/5 flex items-center justify-between">
             ...
             <button
               ...
               className="px-8 py-3 rounded-2xl bg-white text-black font-bold hover:bg-gray-200 transition-all disabled:opacity-20 flex items-center gap-2"
             >
      ...
      <AnimatePresence>
        {isFinishing && (
          ...
          >
            <div className="w-20 h-20 rounded-full bg-purple-500/10 border border-purple-500/20 flex items-center justify-center mb-8">
              <Loader2 className="w-10 h-10 text-purple-500 animate-spin" />
            </div>
            <h2 className="text-3xl font-bold mb-4">Generating Your Report</h2>
            <p className="text-white/40 max-w-sm">Please wait while our AI analyzes your responses and behavior...</p>
          </motion.div>
        )}
      </AnimatePresence>
    </div>
